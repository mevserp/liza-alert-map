  ...
  public async parse(): Promise<ActiveSearchModel.View> {
    ...
    const postsTexts: string[] =
      (await page.evaluate(() => {
        ...
        const postContentElements: HTMLDivElement[] = postElements
          .map((item: HTMLDivElement) => item.querySelector<HTMLDivElement>(".content"))
          .filter((item: HTMLDivElement | null): item is HTMLDivElement => Boolean(item));

        return postContentElements
          .map((item: HTMLDivElement) => item.textContent)
          .filter((textContent: string | null): textContent is string => Boolean(textContent));
      })) ?? [];

    await browser.close();

    const regExpPattern: RegExp = new RegExp(/\d\d[.]\d\d\d\d\d\d[,]\d\d[.]\d\d\d\d\d\d/g);
    const texts: RegExpMatchArray[] = postsTexts
      .map((item: string) => item.trim().split(" ").join("").match(regExpPattern))
      .filter((regExpArray: RegExpMatchArray | null): regExpArray is RegExpMatchArray => Boolean(regExpArray));
