...
export class ActiveSearchUrlsParser {
  public async parse(): Promise<string[]> {
    ...
    const linkElement: puppeteer.ElementHandle<HTMLLinkElement> | null = await page.waitForXPath(
      `//a[contains(., '${ACTIVE_SEARCH_LIST_LINK_TEXT}')]`
    );
    ...
    const activeSearchUrls: string[] =
      (await page.evaluate(() => {
        const SEARCH_LINKS_CSS_CLASS: string = "topictitle";
        const nodes: NodeListOf<HTMLLinkElement> = document.querySelectorAll<HTMLLinkElement>(
          `.${SEARCH_LINKS_CSS_CLASS}`
        );
